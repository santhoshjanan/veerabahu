...
<div class="setup">
  ...
  {/if}
  {#if form?.testStatus}
    <p class:failure={form.testStatus !== 'connected'} class="notice" role="status">
    ...
    >
      ...
      {:else}
        <form method="POST" action="?/access">
          <SecretField name="password" label="Password" autocomplete="new-password" />
          <SecretField
    ...
    >
      <form method="POST" action="?/gatekeeper">
        <label>
          ...
          <select name="type" value={submitted('type', settings?.gatekeeper?.type ?? 'pihole')}>
        ...
        <label>
          ...
          <input name="username" value={submitted('username', settings?.gatekeeper?.username)} />
        </label>
        <SecretField
          name="password"
          label="Password"
      ...
      <form method="POST" action="?/sources">
        <fieldset>
          ...
          <label>
            ...
            <textarea name="curatedListUrls" rows="3">{submitted('curatedListUrls', settings?.curatedListUrls?.join('\n'))}</textarea>
          </label>
        </fieldset>
        ...
        <fieldset>
          ...
          <label><span>Endpoint</span><input name="metadefenderBaseUrl" type="url" value={submitted('metadefenderBaseUrl', settings?.sources.metadefender.baseUrl)} /></label>
          <SecretField name="metadefenderApiKey" label="API key" configured={settings?.sources.metadefender.secretConfigured} />
        </fieldset>
        ...
        <fieldset>
          ...
          <label><span>Endpoint</span><input name="aiBaseUrl" type="url" value={submitted('aiBaseUrl', settings?.sources.ai.baseUrl)} /></label>
          <label><span>Model</span><input name="aiModel" value={submitted('aiModel', settings?.sources.ai.model)} /></label>
          <SecretField name="aiApiKey" label="API key" configured={settings?.sources.ai.secretConfigured} />
        </fieldset>
        ...
        <fieldset>
          ...
          <label><span>Endpoint</span><input name="virustotalBaseUrl" type="url" value={submitted('virustotalBaseUrl', settings?.sources.virustotal.baseUrl)} /></label>
          <SecretField name="virustotalApiKey" label="API key" configured={settings?.sources.virustotal.secretConfigured} />
        </fieldset>
        ...
        {#each sourceNames as [source, label, prefix]}
          <fieldset class="quota">
            ...
            {#each [['PerMinute', 'Per minute'], ['PerDay', 'Per day'], ['PerMonth', 'Per month']] as [suffix, labelText]}
              <label><span>{labelText}</span><input name={`${prefix}${suffix}`} type="number" min="0" step="any" value={submitted(`${prefix}${suffix}`, settings?.quotas[source][`per${suffix.slice(3)}`])} /></label>
            {/each}
            <label><span>Weight</span><input name={`${prefix}Weight`} type="number" min="0" step="any" required value={submitted(`${prefix}Weight`, settings?.weights[source])} /></label>
          </fieldset>
        {/each}
        <fieldset>
          <legend>AI cost</legend>
          <div class="grid">
            <label><span>Daily ceiling (USD)</span><input name="aiDailyCostCeilingUsd" type="number" min="0" step="any" value={submitted('aiDailyCostCeilingUsd', settings?.quotas.ai.dailyCostCeilingUsd)} /></label>
            <label><span>Input / million tokens</span><input name="aiPriceInputPerMTok" type="number" min="0" step="any" value={submitted('aiPriceInputPerMTok', settings?.sources.ai.priceInputPerMTok)} /></label>
            <label><span>Output / million tokens</span><input name="aiPriceOutputPerMTok" type="number" min="0" step="any" value={submitted('aiPriceOutputPerMTok', settings?.sources.ai.priceOutputPerMTok)} /></label>
          </div>
        </fieldset>
        <div class="actions">
  ...
  {:else}
    ...
    >
      <dl>
        ...
        <div><dt>Sources</dt><dd>{sourceNames.filter(([source]) => settings?.sources[source].enabled).map(([, label]) => label).join(', ')}</dd></div>
      </dl>
      <aside>
...
<style>
  ...
  .notice.failure, .notice.error { border-color: var(--vb-danger); color: var(--vb-danger); }
