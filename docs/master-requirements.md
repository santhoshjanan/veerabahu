...
| ----- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** | **Core enrichment pipeline (MVP)** | Pi-hole read adapter → pull resolved domains → assess reputation via 3 parallel sources (local curated lists + MetaDefender + AI) → ranked candidate list → HITL review → **publish blocklist HTTP endpoint** for the gatekeeper to subscribe to. Includes DB schema, job runner, quota governor (§9), and consumption-health logging. | **Implemented.** Pi-hole and AdGuard adapters, persistent ingestion/assessment, HITL review, publisher, audit rows, and consumption health are covered by tests. PostgreSQL remains supported by the schema/migrations but is not exercised in CI. |
| 2     | Dashboard & audit-trail UI         | Read-only view over pipeline data.                                                                                                                                                                                                                                                                                                     | **Implemented.** Dashboard, review queue, domain detail, and audit views are live.                                                                                                                                                                 |
| 3     | AI observability & cost tracking   | Token/cost metrics and call traces for AI reputation calls.                                                                                                                                                                                                                                                                            | **Partial.** Per-verdict token cost feeds dashboard totals; a dedicated call-trace view is pending.                                                                                                                                                |
| 4     | Settings + onboarding wizard       | **First task: move config + secrets from env into the DB** (env demoted to bootstrap/override) — implies **encryption-at-rest for DB-stored secrets**. Then: connection/credential config (gatekeeper API + per-reputation-source API key/auth/endpoint), quota-governor limits, weights, first-run wizard.                            | **Implemented.** Onboarding and Settings manage encrypted DB-backed credentials, sources, quotas, weights, and scheduler activation.                                                                                                               |
| 5     | Consent-based anonymized telemetry | Opt-in usage collection.                                                                                                                                                                                                                                                                                                               | Backlog                                                                                                                                                                                                                                            |
| —     | CI + test harness                  | Cross-cutting. Set up minimally alongside #1.                                                                                                                                                                                                                                                                                          | **Implemented (SQLite).** CI runs lint, type-check, build, coverage-gated unit tests, and Playwright E2E; PostgreSQL CI remains pending.                                                                                                           |
...
- **2026-09-06** — Reconciled sub-project status with the implementation: the core
  pipeline, dashboard/audit UI, and encrypted settings/onboarding are implemented.
  AI call traces and consent telemetry remain pending; CI does not yet run PostgreSQL.
