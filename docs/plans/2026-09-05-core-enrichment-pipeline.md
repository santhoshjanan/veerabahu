...
export const domainClients = sqliteTable(
  'domain_clients',
  {
    domainId: integer('domain_id')
      .notNull()
...
export const domainClients = pgTable(
  'domain_clients',
  {
    domainId: integer('domain_id')
      .notNull()
...
export type VerdictRow = {
  ...
  domainId: number;
  source: SourceName;
  verdict: 'block' | 'allow' | 'unsure' | 'error';
  confidence: number;
...
export async function upsertObservedDomain(
  ...
  await db
    .insert(schema.domainClients)
    .values({ domainId, clientId: args.clientId })
    .onConflictDoNothing();

  const [{ cnt }] = await db
    .select({ cnt: sql<number>`count(*)` })
    .from(schema.domainClients)
    .where(eq(schema.domainClients.domainId, domainId));
...
export async function setIngestState(
  ...
  schema: any,
  patch: Partial<
    Pick<IngestStateRow, 'cursor' | 'lastIngestAt' | 'firstRunDone'>
  >
): Promise<void> {
...
- Produces:
  ...
  - `src/lib/server/adapters/gatekeeper/pihole.ts`:
    ...
    export function makePiholeAdapter(cfg: {
      ...
      _authedFetch(path: string, init?: RequestInit): Promise<Response>;
    };
    ```
  ...
  "queries": [
    {
      ...
      "cname": null,
      "status": "FORWARDED",
      "client": { "ip": "192.168.1.20", "name": "laptop" },
      "reply": { "type": "IP", "time": 12 },
      "upstream": "1.1.1.1#53",
      "list_id": null
    },
    {
      ...
      "cname": null,
      "status": "GRAVITY",
      "client": { "ip": "192.168.1.21", "name": null },
      "reply": { "type": "NULL", "time": 1 },
      "upstream": null,
      "list_id": 4
    },
    {
      ...
      "cname": "tracker.evil.test",
      "status": "DENYLIST_CNAME",
      "client": { "ip": "192.168.1.22", "name": "phone" },
      "reply": { "type": "NULL", "time": 2 },
      "upstream": null,
      "list_id": null
    }
  ],
...
- Produces:
  ...
  export function makeDnsLookup(deps?: {
    resolver?: {
      ...
      resolveCname(d: string): Promise<string[]>;
      resolveNs(d: string): Promise<string[]>;
    };
...
describe('makeDnsLookup', () => {
  ...
  it('tolerates per-record failures (returns empty arrays, not null)', async () => {
    ...
    expect(await lookup('x.com')).toEqual({
      a: [],
      cname: [],
      ns: ['ns1.example.net']
    });
...
export function makeWhoisLookup(deps?: {
  ...
  return async (domain: string): Promise<WhoisInfo | null> => {
    ...
    const block = (
      raw && typeof raw === 'object' ? Object.values(raw)[0] : null
...
// registry.ts
export function buildEnabledSources(
  cfg: Config,
  db: any,
  schema: any
): {
...
**AI cost:** `costUsd = (inputTokens/1e6)*priceInput + (outputTokens/1e6)*priceOutput` when both prices set, else `0`. The `SourceVerdict.usage` carries `{ inputTokens, outputTokens, costUsd }`. (The daily-ceiling _enforcement_ is the governor's job in Task 12; this source only reports cost.)
...
const input: AssessmentInput = {
  ...
  enrichment: {
    whois: { ageDays: 20, registrar: 'NameCheap' },
    dns: { a: ['1.1.1.1'], cname: ['x'], ns: ['ns1'] }
  }
};
...
export function makeAiSource(cfg: {
  ...
  return {
    ...
    limits: {
      ...
      perDay: null,
      perMonth: null,
      dailyCostCeilingUsd: cfg.dailyCostCeilingUsd
    },
...
import {
  ...
  decideState,
  AUTO_CLEAR_ABOVE
} from '$lib/server/scoring/score';

const v = (source: any, verdict: any, confidence: number) => ({
  source,
  verdict,
  confidence
});
...
describe('computeScore', () => {
  it('returns null when only unsure/error votes exist', () => {
    expect(
      computeScore([v('curated_list', 'unsure', 0), v('ai', 'error', 0)])
    ).toBeNull();
  });
  ...
  it('clamps into [-1, 1]', () => {
    expect(
      computeScore([
        v('curated_list', 'block', 1),
        v('metadefender', 'block', 1),
        v('virustotal', 'block', 1)
      ])
    ).toBe(-1);
  });
});
...
export function decideState(args: {
  ...
  if (allIn || args.nowMs - args.domain.firstSeen > args.maxReviewWaitMs)
...
- Produces:
  ...
  export function canCall(
    ...
    | { ok: true }
    | { ok: false; reason: 'minute' | 'day' | 'month' | 'paused' | 'wait' };
...
export function afterCall(
  ...
  const hitCeiling =
    l.dailyCostCeilingUsd != null && dayCostUsd >= l.dailyCostCeilingUsd;
  return {
    ...s,
    tokens,
    lastCallAt: nowMs,
...
export async function listReview(
  ...
  for (const d of domains) {
    ...
    out.push({
      ...
      score: d.score,
      hitCount: d.hitCount,
      distinctClientCount: d.distinctClientCount,
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const off = subscribe((domainId) => {
        controller.enqueue(
          enc.encode(`data: ${JSON.stringify({ domainId })}\n\n`)
        );
      });
...
export async function startBackground(opts?: {
  ...
  const enrich = async (domain: string) => ({
    whois: await whois(domain),
    dns: await dns(domain)
  });
