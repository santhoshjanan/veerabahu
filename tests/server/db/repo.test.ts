...
describe('repo.upsertObservedDomain', () => {
  ...
  it('never lets last_seen go backwards', async () => {
    const t = await makeTestDb();
    closer = t.close;
    await repo.upsertObservedDomain(t.db, t.schema, { domain: 'x.com', clientId: 'c1', at: 500 });
    await repo.upsertObservedDomain(t.db, t.schema, { domain: 'x.com', clientId: 'c1', at: 100 });
    const row = await repo.getDomainByName(t.db, t.schema, 'x.com');
...
describe('repo.decideDomain', () => {
  ...
  it('reject sets state rejected', async () => {
    ...
    const row = await repo.getDomainById(t.db, t.schema, domainId);
    expect(row).toMatchObject({ state: 'rejected', decidedAt: 5, decisionNote: null });
...
describe('repo.logBlocklistFetch', () => {
  it('appends a fetch-log row', async () => {
    const t = await makeTestDb();
    closer = t.close;
    await repo.logBlocklistFetch(t.db, t.schema, { at: 10, ip: '10.0.0.1', userAgent: null, status: 200 });
    const [row] = await t.db.select().from(t.schema.blocklistFetchLog);
    expect(row).toMatchObject({ at: 10, ip: '10.0.0.1', userAgent: null, status: 200 });
  });
});
  ...
  it('returns pending_review domains ranked by hit_count then first_seen, with limit/offset', async () => {
    ...
    for (const [d, hits, at] of [
      ...
      for (let i = 0; i < hits; i++)
        await repo.upsertObservedDomain(t.db, t.schema, { domain: d, clientId: 'c', at });
      const row = await repo.getDomainByName(t.db, t.schema, d);
      await repo.setDomainScoreAndState(t.db, t.schema, row!.id, null, 'pending_review');
    }
